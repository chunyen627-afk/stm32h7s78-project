...
### 補一刀（2026-08-27）：「沒聲音＋一直斷線重連」＝抽送開關忘了開回來

凍結修好後使用者實測：**耳機沒聲音、一直斷開重連。** 不是時脈、
不是新的凍結（黑盒子顯示相簿活得好好的、類別就緒）——是
`USB_PUMP_ENABLE` 還停在凍結二分法時期的 0。抽送關著的話，
**開播序列（解靜音→取樣率→Play）住在 TIM7 中斷裡，根本沒人執行**：
串流永遠不會開始 → 沒聲音；而 dongle 收不到等時串流，省電機制就把
無線鏈路一直斷開重建 → 耳機不停播報「已連線／已中斷」。

**「一直斷線重連」不一定是時脈問題 —— 沒有串流本身就會造成它。**

開回 1 之後用治具驗證（這次治具加了「等類別就緒才開播」的閘，
不然 autoplay 會搶在列舉前面、音訊退回 I2S，**USB 串流路徑根本沒被
測到** —— 上一輪的 308 秒驗證就是這樣漏掉的）：310 秒、77 次滑動、
US_PLAYING 全程、欠載 0、繞回 6.15 次/秒（正好是 192KB/s 的即時
消耗率）、影片 23.8fps、零凍結。

順帶更正：`UB_OVER`（172 號）的註解說「應該永遠是 0」已過時 ——
繞回所有權搬進 TIM7 之後它數的是**正常的緩衝繞回**，健康值就是
每 160ms 一次。

### 還沒做的（接續第三階段）

1. 使用者實聽驗收：插 dongle 播影片、滑動、暫停（音畫同步也聽一下 ——
   治具量到固定偏移的數字被滑動的暫態污染，只能靠耳朵判定）
2. 輸出選擇的邊界情形（播放中拔/插 dongle）與螢幕音量控制
3. 與隨身碟模式（USB HS / CN18）並存的回歸測試


## 冷上電後 dongle 無聲（2026-08-27）：查清楚了，決定不修

**症狀**：板子冷上電（dongle 插著一起通電）後播影片，主機端每個環節都
正常 —— 類別 3 秒掛上、US_PLAYING、封包 192KB/s 一個不少、黑盒子全綠
—— 但耳機無聲。重插 dongle 就好。

**注意**：等時端點沒有 ACK，「封包都送出去了」證明不了對方有收好 ——
主機端的計數器在這種問題上是盲的。
...
| 補救動作 | 時間點 | 結果 |
|---|---|---|
| USBH_ReEnumerate | 第 12 秒 | 無效 |
| MCU 自我重置（NVIC_SystemReset）| 第 10 秒 | 無效 |
| 埠掛起 2.5 秒再列舉 | 第 10 秒 | 測試無效（suspend 下重啟卡住，第二次列舉根本沒發生）|
| 使用者重插 dongle | >30 秒 | **有效** |
| SWD 觸發的 MCU 重置 | 36 秒 | **有效** |
| 晚期重新列舉 | >36 秒 | **有效** |

**結論：唯一的變因是時間。** dongle 通電後要約半分鐘才真正就緒，
太早列舉綁出來的音訊路徑是壞的，跟主機端用哪種方式重來無關。

### 排除掉的（都實測，不要重查）

- **時脈**：黑盒子 193 號證明冷上電走的就是 HSI48+CRS（LSE 167ms 就
  緒）、CRS 的 SYNCOK 有鎖住 —— 不是退回 PLL3Q。
- **列舉被開機掃卡餓死**：類別 3.0 秒就掛上，比掃卡還早。
- **主機端狀態**：兩次列舉（190 號計數）、斷線計數、串流狀態全部正常。

### 為什麼不修（使用者的取捨）

有效的修法只剩「開機約 40 秒後補一次列舉」（無感版做好也驗過機制會
發動）。但使用者評估：冷上電本來就少見，**手動重插 2 秒比等 40 秒快**；
自動重置版更糟 —— 掃卡要跑兩次。所以**維持現狀：冷上電後重插一次
dongle 即可**。40 秒版的程式碼在 git 歷史裡（找 USBH_ReEnumerate），
哪天要撿回來就在。
...
| 190 | 類別掛上：(次數<<24)\|tick |
| 191 | 斷線次數（本輪開機）|
| 193 | USB 時脈路徑：0xC1=HSI48+CRS、0xC2=退回 PLL3Q（`usbh_glue.c` 寫入）|
