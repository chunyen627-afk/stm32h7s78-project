...
def patch_main():
    ...
    s = s.replace(
        ...
        "\n"
        "      *flag = 0u;\n"
        "      /* **清完一定要寫回記憶體。** 這裡如果 D-Cache 是開的，寫入會留在\n"
        "       * 快取裡；而隨身碟 app 的 SD 讀取路徑有 SCB_InvalidateDCache()，\n"
        "       * 整體失效會把髒快取行**直接丟掉**，於是 SRAM 裡留的還是舊的暗號\n"
        "       * -> 下次開機又跳過去 -> 永遠回不了相簿。實測踩過：相簿開機三次、\n"
        "       * 第三次仍讀到 0x55534244，而切換次數始終是 1。 */\n"
        "      SCB_CleanDCache_by_Addr((uint32_t *)flag, 32);\n"
        "      __DSB();\n"
        "\n"
