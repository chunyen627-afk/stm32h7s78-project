...
if [ ! -f "$ROOT/core/font_zh.c" ]; then
    echo "==> 產生中文字型表（21386 字，約需一分鐘）"
    (cd "$ROOT" && python tools/genfont.py >/dev/null)
fi
...
echo "==> 複製相簿原始碼"
mkdir -p "$PROJ/Appli/Album"
cp "$ROOT"/core/*.c "$ROOT"/core/*.h "$PROJ/Appli/Album/"
cp "$ROOT"/app_src/*.c "$ROOT"/app_src/*.h "$PROJ/Appli/Album/"
cp "$REPO"/shared/gfx.c "$REPO"/shared/gfx.h "$REPO"/shared/xspi_psram.c "$PROJ/Appli/Album/"
