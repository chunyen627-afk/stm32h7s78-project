...
- **勾選最上層資料夾**播放（最多 64 個），選到的資料夾底下**所有子層**的
  照片都會撈進來，上限 8,192 張
- **隨機播放**（Fisher-Yates 洗牌，每輪重洗，觸控注入熵），間隔 **2~5 秒**
  可調；解碼與顯示重疊，間隔設定值就是實際展示時間
- **輕點暫停**：跳出操作列（上一張 / 繼續 / 下一張 / 選單），一秒後自動
  收起，再點喚醒；暫停中可連續翻閱
- **USER 鍵待機**：按一下關螢幕並暫停，再按一下接著播
- **記憶卡熱插拔**：拔卡自動卸載並提示，插回自動重新掃描（含進度顯示）
- **看門狗**（16 秒）：記憶卡沒回應時自動重置並提示拔卡重插，不會卡死在
  「載入中」
- 對記憶卡**只讀不寫**，磁碟層直接回報寫入保護
- JPEG baseline，4:4:4 / 4:2:2 / 4:2:0 都支援，單張上限約 3.4MP
