...
**透過 ST-LINK 覆寫 SD 卡上已存在的 `video.bin` 會失敗。**

- 建立新檔案沒問題（實測寫入 4.58MB 完全成功）
- 但 `f_open(FA_CREATE_ALWAYS)` 在檔案已存在時要先釋放整條簇鏈，
  這會產生大量**單磁區的 FAT 寫入**，連續約 130 次之後卡片就不再回應指令
- 現場：`BSP_SD_WriteBlocks` 回 -4，`HAL_SD_GetError` = 4（`CMD_RSP_TIMEOUT`），
  `hsd.State` = READY（軟體狀態正常），卡片回報 BUSY，失敗磁區在 6124~6126 附近
- 已排除：唯讀保護（實際寫入有發生）、HAL 狀態機卡死（State 是 READY）、
  快取一致性（HAL 的寫入是 CPU 輪詢 FIFO，不走 DMA）
- 讀取完全正常（數千次無誤），大塊循序寫入也正常

還沒查明是卡片本身、SDMMC 時脈餘裕，還是 busy 處理不足。
**在查清楚之前不要拿有資料的卡反覆試** —— 失敗發生在 FAT 區，有破壞檔案系統的風險。

## 待辦

1. 查明上述 SD 寫入問題（建議先拿一張空白測試卡）
