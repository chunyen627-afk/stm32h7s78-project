...
| [projects/album](projects/album) | 電子相簿，讀 SD 卡照片隨機輪播，可勾選資料夾 |
...
**外部 NOR Flash 在 `0x70170466` 有一個不穩定位元**。每次燒錄都會出現，
目前落在查表資料裡，無害。album 的 `flash.sh` 會整份讀回自己比對，並指出
不符的位元組落在哪個符號 —— 落在查表資料可以忽略，落在 `.text` 的函式不行。
